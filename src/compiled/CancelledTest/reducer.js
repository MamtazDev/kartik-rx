...
const INITIAL_APP_STATE = {
  loaded: false,
  pageReset: 0,
  ...helperFns.INITIAL_STATE({cancelled_only: true}),
};
...
export const actionHandlersMap = {
  ...
  [ACTION_TYPES.SUMMARY_DATA_LOAD]: (payload, {dispatch}) => {
    helperFns.actionHandlerUtils.getCancelledTestData(
      payload, {dispatch}, {
...
function baseReducer(state = {...INITIAL_APP_STATE}, action) {
  ...
  case ACTION_TYPES.SUMMARY_TABLE_INIT: return helperFns.actionTypesUtils.cancelledTestDataloadFn(state, action);
  case ACTION_TYPES.SUMMARY_DATA_LOAD_SUCCEEDED: return helperFns.actionTypesUtils.cancelledTestDataloadSuccessFn(state, action);
